...
export default function useTournament(tournamentId) {
  ...
  const fetchTournament = useCallback(async () => {
    ...
    // Asegurar arrays y resultados iniciales
    ...
    // Inicializar matchResults con "saved" si ya tiene sets
    const init = {};
    [...t.groups, ...t.rounds].forEach(section => {
      section.matches.forEach(m => {
        init[m._id] = { ...m.result, saved: Array.isArray(m.result.sets) && m.result.sets.length > 0 };
      });
    });
  ...
  const onSaveResult = useCallback(async (matchId, result) => {
    try {
      // protejo tournament.rounds cuando es null
